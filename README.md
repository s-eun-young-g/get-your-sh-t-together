...
A personal productivity app with layers instead of one long list. New here? [The guide](GUIDE.md) explains every tab and how to get your own copy; it also comes as [a PDF](guide.pdf) for sharing. Built for a brain that needs the day assembled for it, dreaded tasks handled specially, and structure that grows without nagging.
...
- **Model**: models with nestable categories and cards, plus autosaving notes. Each category opens into a full page with unlimited room to recurse. Models start empty; the structure is yours, or import a Notion export (markdown and csv zip) to send pages to the inbox and turn any database into a model, grouped by the column you pick.
- **Learn**: dependency trees you build yourself; nothing is pre-populated. Add categories and items by hand, or import a ChatGPT or Claude export. Track pages list everything, and any item or category can be deleted or merged; clicking an item's title opens its own notes and sources (as many as you want). Mark any number of items "learning now". Day streaks log automatically when you complete items. Import your ChatGPT or Claude data export and map old learning chats into the trees.
